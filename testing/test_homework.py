...
#
# The following enviroment variables are used to run the homemade_dropbox client & server.
# export CLIENT_CMD='go run ~/homemade_dropbox/client/main.go /tmp/dropbox/client'
# export SERVER_CMD='go run ~/homemade_dropbox/server/main.go /tmp/dropbox/server'
#
...
class TestBasic(TestCase):
    ...
    def test_add_file_within_subdir(self):
        mkdir(join(self.cpath, 'newdir'))
        sleep(1.0)
        spit(join(self.cpath+"/newdir", 'newfile.txt'), 'contents')        
        assert_paths_in_sync(self.cpath, self.spath)
    
    def test_add_dir_within_subdir(self):
        mkdir(join(self.cpath, 'newdir'))
        sleep(1.0)
        mkdir(join(self.cpath+"/newdir", 'nesteddir'))        
        assert_paths_in_sync(self.cpath, self.spath)
    
    def test_add_and_remove_file_within_subdir(self):
        mkdir(join(self.cpath, 'newdir'))
        sleep(1.0)
        spit(join(self.cpath+"/newdir", 'newfile.txt'), 'contents')
        assert_paths_in_sync(self.cpath, self.spath)

        remove(join(self.cpath+"/newdir", 'newfile.txt'))
        assert_paths_in_sync(self.cpath, self.spath)
